...
void SignalHandler(int sig) {
  ...
  if (output_file && output_transformed_file) {
    ...
    rewind(output_transformed_file);  // Go back to the beginning of the raw
                                      // data file
...
void CreateSymbolicLink() {
  const char *sourcePath =
      "../../mouse_saver/build/mouse_data.dat";  // Source file in
                                                 // mouse_saver/build
  const char *targetPath =
      "../../mouse_plotter/build/mouse_data.dat";  // Symbolic link in
                                                   // mouse_plotter/build
